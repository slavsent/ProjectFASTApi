...
def menu_create(payload: MenuBaseSchema, db: Session = Depends(get_db)):
...
def menu_update(menu_id: str, payload: MenuBaseSchema, db: Session = Depends(get_db)):
    # try:
    #    int(menu_id)
    # except ValueError:
...
def menu_read(menu_id: str, db: Session = Depends(get_db)):
    if menu_id == 'null':
        client.delete('menus')
        return {"detail": "menu not found"}
    else:
...
def menu_delete(menu_id: str, db: Session = Depends(get_db)):
    if menu_id == 'null':
        client.delete('menus')
        return {"detail": "menu not found"}
    else:
